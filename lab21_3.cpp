...
int main(){
	...
	while(getline(student_file,textline)){
		...
		char gender[1];
		float gpa;
		sscanf(textline.c_str(), "%[^,], %i, %c, %f", name ,&id, gender, &gpa);
		s.name = name;
		s.id = id;
		s.gender = *gender;
		s.gpa = gpa;
		...
		if(state == 1){
			...
			if(textline == "> Students"){
			    ...
			    if(allcourses.size() == 1)
					allcourses[0].lecture_list.push_back(textline);
				else if(allcourses.size() == 2)
					allcourses[1].lecture_list.push_back(textline);
				else
					allcourses[2].lecture_list.push_back(textline);
